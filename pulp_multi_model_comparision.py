...
# 比较多种求解器
solvers = {
    #"SCIP": SCIP_CMD(msg=False),
    #"CBC": PULP_CBC_CMD(msg=False), #速度暂时和SCIP差不多，求解结果一致
    "GLPK": GLPK_CMD(msg=True) #速度最慢，可能是应用场景不适配。
    # "Gurobi": pulp.GUROBI_CMD(msg=True), #需要安装Gurobi软件，且需要在系统路径中添加Gurobi可执行文件路径。
    #"CPLEX": pulp.CPLEX_CMD(msg=False) #需要安装CPLEX软件，且需要在系统路径中添加CPLEX可执行文件路径。
...
for solver_name, solver in solvers.items():
    ...
    if pulp.LpStatus[model.status] == 'Optimal':
        ...
        print(f"{solver_name} 求解时间：", solve_time, "秒")
        
        # 记录结果
        results_summary[solver_name] = {"Total Cost": total_cost_value, "Solve Time": solve_time}
    else:
        print(f"{solver_name} 未找到可行解")
        results_summary[solver_name] = {"Total Cost": None, "Solve Time": solve_time}
